...
bool JsonObj::Get(const QString& strKey, QString& strValue) const
{
    cJSON* pJsonStruct = NULL;
    if (m_pJsonData != NULL)
    {
        if (m_pJsonData->type == cJSON_Object)
        {
            pJsonStruct = cJSON_GetObjectItem(m_pJsonData, strKey.toUtf8().data());
        }
    }
    else if (m_pExternJsonDataRef != NULL)
    {
        if(m_pExternJsonDataRef->type == cJSON_Object)
        {
            pJsonStruct = cJSON_GetObjectItem(m_pExternJsonDataRef, strKey.toUtf8().data());
        }
    }
    if (pJsonStruct == NULL)
    {
    ...
    strValue = QString::fromUtf8(pJsonStruct->valuestring);
    return(true);
}
